...
function studentlist_ALL(Web $w) {
    ...
    $table_headers = ['Participant Name', 'Status', 'Main Contact', 'Actions'];
    ...
    if (!empty($students)) {
        foreach ($students as $student) {
            ...
            $row[] = $student->getMainContact()->getFullName();
            $actions = [];
            $actions[] = Html::b('/school-manager/studentview/' . $student->id, 'View');
            if ($loggedInUser->hasRole('school_manager')) {
